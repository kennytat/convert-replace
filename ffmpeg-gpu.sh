...
ffmpegVideoGPU() {
	ffmpeg -progress pipe:1 -stats_period 0.5 -vsync 0 -hwaccel_device ${gpuNum} -hwaccel cuvid -c:v h264_cuvid -i "${inPath}" \
		-filter_complex \
...
[v4]scale_npp=w=640:h=360:force_original_aspect_ratio=decrease[v4out]" \
		-map "[v1out]" -c:v h264_nvenc -gpu ${gpuNum} -preset:v p7 -tune:v hq -rc:v vbr -cq:v 19 -b:v:0 "$rate1080"K -maxrate:v:0 "$rate1080"K -bufsize:v:0 "$buf1080"K -g 48 -sc_threshold 0 -keyint_min 48 \
		-map "[v2out]" -c:v h264_nvenc -gpu ${gpuNum} -preset:v p7 -tune:v hq -rc:v vbr -cq:v 19 -b:v:1 "$rate720"K -maxrate:v:1 "$rate720"K -bufsize:v:1 "$buf720"K -g 48 -sc_threshold 0 -keyint_min 48 \
		-map "[v3out]" -c:v h264_nvenc -gpu ${gpuNum} -preset:v p7 -tune:v hq -rc:v vbr -cq:v 19 -b:v:2 "$rate480"K -maxrate:v:2 "$rate480"K -bufsize:v:2 "$buf480"K -g 48 -sc_threshold 0 -keyint_min 48 \
		-map "[v4out]" -c:v h264_nvenc -gpu ${gpuNum} -preset:v p7 -tune:v hq -rc:v vbr -cq:v 19 -b:v:3 "$rate360"K -maxrate:v:3 "$rate360"K -bufsize:v:3 "$buf360"K -g 48 -sc_threshold 0 -keyint_min 48 \
